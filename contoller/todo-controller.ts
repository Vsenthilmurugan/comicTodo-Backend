...
export const todoList = async (req: Request, res: Response, next:NextFunction) => {
...
export const todoItem =  async (req: Request, res: Response, next:NextFunction) => {
    const todoId = req.params.todoId;
    const objectId = new ObjectId(todoId);
    const todoItem = await todoCollection.findOne({ _id:objectId})
  if(!todoItem){
    return next(new HttpError('could not find todo Item for the given id',404));
  }
    res.json({data:todoItem});
}
...
export const updateTodoItem = async (req: Request, res: Response, next:NextFunction) =>{
  const errors = validationResult(req);
  if(!errors.isEmpty()){
    const errorMessages = errors.array().map((error) => error.msg);
    const errorMessage = errorMessages.join(', ');
  ...
  const result = await todoCollection.updateOne({ _id:objectId}, { $set: todoData })
